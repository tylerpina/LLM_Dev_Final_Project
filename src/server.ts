...
if (process.env.OPENAI_API_KEY) {
	...
	personalizationService.initialize()
		...
		.catch(err => logger.error('Failed to initialize personalization service:', err));
}

// Initialize database and headline fetcher
const databaseService = new DatabaseService();
const headlineFetcherService = new HeadlineFetcherService(databaseService, mcpServer);
...
app.post('/ask', async (req, res) => {
	...
	const { query, style, userId } = req.body;
	
	logger.info('AI query received', { query, style, userId, timestamp: new Date().toISOString() });
...
app.post('/personalize/index', async (req, res) => {
	try {
		...
		await personalizationService.indexArticle(articleId, title, content, metadata);
...
app.post('/personalize/track', async (req, res) => {
	try {
		...
		const { userId, query, articleId, articleTitle, articleContent, interactionType } = req.body;
		...
		logger.info('Interaction tracked successfully', { userId, interactionType });
...
app.get('/headlines/recent/:hours', (req, res) => {
	try {
		...
		logger.info('Headlines by time range retrieved', { hours, count: headlines.length });
...
app.get('/headlines/search', (req, res) => {
	try {
		...
		logger.info('Headlines search completed', { keyword, count: headlines.length });
...
app.post('/headlines/fetch', async (_req, res) => {
	try {
		...
		headlineFetcherService.fetchNow()
...
app.listen(port, () => {
	...
	console.log(`🎯 Personalization: ${personalizationService ? 'ENABLED' : 'DISABLED (set OPENAI_API_KEY)'}`);
